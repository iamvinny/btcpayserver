...
namespace BTCPayServer.Controllers
{
    public partial class InvoiceController
    {
        ...
        public async Task<IActionResult> Refund(string invoiceId, CancellationToken cancellationToken)
        {
            ...
            if (invoice is null)
                return NotFound();
            if (invoice.PullPaymentDataId is null && GetUserId() is null)
                return NotFound();
            if (!CanRefund(invoice.GetInvoiceState()))
                return NotFound();
            if (invoice.PullPaymentDataId is string ppId)
            {
            ...
            else
            {
                ...
                var options = invoice.GetBlob(_NetworkProvider).GetPaymentMethods()
                    .Select(o => o.GetId())
                    .Where(o => o.PaymentType == PaymentTypes.BTCLike)
                    .ToList();
                ...
                refund.SelectedPaymentMethod = defaultRefund ?? options.First().CryptoCode;
        ...
        public async Task<IActionResult> Refund(string invoiceId, RefundModel model, CancellationToken cancellationToken)
        {
            ...
            if (model.SelectedRefundOption is null)
            {
                ...
                var paidCurrency = paymentMethod.Calculate().Paid.ToDecimal(MoneyUnit.BTC) * paymentMethod.Rate;
                model.CryptoAmountThen = paidCurrency / paymentMethod.Rate;
                ...
                model.CryptoAmountNow = paidCurrency / rateResult.BidAsk.Ask;
            ...
            else
            {
                ...
                {
                    Message = "You can share this page with the person desiring a refund",
                    Severity = StatusMessageModel.StatusSeverity.Success
                });
                (await ctx.Invoices.FindAsync(invoice.Id)).PullPaymentDataId = ppId;
